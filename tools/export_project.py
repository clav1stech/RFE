...
# Dossiers ignorés dans TOUS les profils (artefacts, environnements, config locale)
EXCLUDED_DIRS_ALWAYS = {
    ...
    "build",
    "dist",
    "output",  # factures générées à l'exécution (cf. .gitignore)
    ".idea",
    ".vscode",
}
...
# Fichiers ignorés uniquement en profil --ai (faible densité d'information pour une IA)
EXCLUDED_FILES_AI = {"uv.lock"}
...
# faible densité d'information pour une IA mais nécessaire à une sauvegarde fidèle)
...
def collect_files(profile, only=None):
    ...
    profile == "ai"     → exclut tools/ et uv.lock (analyse curée).
    profile == "backup" → tout le code source, hors artefacts/binaires (sauvegarde fidèle).
    ...
    for path in sorted(project_dir.rglob("*")):
        ...
        if path.name in excluded_files:
            continue
        # .gitignore n'a pas d'extension : on retombe sur le nom
